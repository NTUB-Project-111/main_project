...
class Call extends ChangeNotifier {
  ...
  //   return map;
  // }
  void createReminders(List<UserReport> userRecords, Map<int, List<UserRemind>> remindsMap) {
    reminders = userRecords.where((record) => record.ifcall == "Y").map((record) {
      return {
        "date": record.date,
        "type": record.type,
        "img": record.photo,
        "isPressed": false,
        "selectedFreq": remindsMap[record.id]?[0].freq,
        "selectedHour": int.parse(remindsMap[record.id]![0].time.split(":")[0]),
        "selectedMinute": int.parse(remindsMap[record.id]![0].time.split(":")[1]),
        "isDeleteView": false,
        "ifcall": true
      };
    }).toList();
